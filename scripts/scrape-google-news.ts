...
import { searchGoogleNews, titleFingerprint, SearchHit } from './search';
...
// on which query fetched it. `titleFingerprint` (shared in ./search) is used as a
// secondary dedup key within a run and as the `notes` field so isDuplicate can catch
// the same article coming in on a future run.

function candidateToReviewEntry(c: SearchHit, index: number): ReviewEntry {
  ...
  return {
    ...
    date_announced: parseDate(c.publishedDate) || new Date().toISOString().slice(0, 10),
    date_reported: parseDate(c.publishedDate) || new Date().toISOString().slice(0, 10),
